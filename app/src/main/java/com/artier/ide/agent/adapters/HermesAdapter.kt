...
class HermesAdapter : AgentAdapter {
    ...
    override val name = "hermes"
    override val version = "1.0.0"
    
    private val command = "hermes"
    ...
    override fun spawn(config: AgentConfig): Flow<AgentEvent> = flow {
        ...
        try {
            ...
            // Point Hermes at local 9Router
            val env = processBuilder.environment()
            AgentRouterEnv.applyTo(env, config.environment)
            if (config.apiKey != null) {
                env["HERMES_API_KEY"] = config.apiKey
            }
    ...
    override fun sendInput(sessionId: String, input: String) {
        // Hermes doesn't support interactive input in non-interactive mode
    }
